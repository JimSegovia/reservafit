...
export default function LandingScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-cream p-0 m-0">
      {/* Background glow */}
      <View className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full bg-primary/[0.04] pointer-events-none" />
      <View className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-primary/[0.06] pointer-events-none" />
      {/* Web Navigation */}
      {isWeb ? (
        <Animated.View entering={FadeIn.duration(200)} className="flex-row justify-between items-center py-5 px-12 bg-white shadow-md z-10 w-full overflow-hidden">
          ...
          <View className="flex-row items-center gap-x-4">
            ...
            <Button
              ...
              variant="outline"
              className="py-2.5 px-8 min-h-0 h-11 !w-auto hover:bg-primary/10"
            />
            <Button
              label="Registrarse"
              onPress={() => router.push('/(auth)/register')}
              variant="primary"
              className="py-2.5 px-8 min-h-0 h-11 !w-auto shadow-md shadow-primary/30"
            />
          </View>
        </Animated.View>
      ) : null}
       ...
       >
         ...
         <View className={`${isWeb ? 'flex-row items-center justify-between gap-x-10' : 'flex-col'}`}>
            ...
            {!isWeb && (
               <Animated.View entering={FadeInDown.duration(250).delay(150)} className="mt-8 gap-y-3 w-full pb-2">
                  <Button
                    ...
                    onPress={() => router.push('/(auth)/login')}
                    variant="outline"
                    className="min-h-0 h-12 hover:bg-primary/10"
                  />
                  <Button
                    label="Registrarse"
                    onPress={() => router.push('/(auth)/register')}
          ...
          <Animated.View entering={ZoomIn.duration(250).delay(100)} className={`items-center justify-center ${isWeb ? 'flex-1' : 'mt-4'}`}>
               <View
                  className={`w-full overflow-hidden bg-gray-200 ${isWeb ? 'aspect-[4/3] rounded-[40px] max-w-[600px]' : 'h-64 rounded-[30px]'}`}
                  style={{
                    shadowColor: '#FF7A00',

...
export class MailService {
  ...
  static async enviarCodigoRestablecimiento(correo: string, codigo: string): Promise<boolean> {
    try {
      ...
      const info = await transporter.sendMail({
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; border-radius: 12px; background-color: #ffffff;">
            ...
            <h2 style="color: #333333; font-size: 20px; border-bottom: 2px solid #FF7A00; padding-bottom: 8px;">Restablece tu contraseña</h2>
            <p style="color: #555555; font-size: 16px; line-height: 1.5;">Hola,</p>
            <p style="color: #555555; font-size: 16px; line-height: 1.5;">Hemos recibido una solicitud para restablecer la contraseña de tu cuenta. Utiliza el siguiente código de verificación (OTP) de 6 dígitos para continuar con el proceso:</p>
            <div style="background-color: #F9FAFB; border: 1px solid #E5E7EB; padding: 20px; text-align: center; border-radius: 8px; margin: 24px 0;">
              <span style="font-size: 36px; font-weight: bold; letter-spacing: 6px; color: #1F2937;">${codigo}</span>
            </div>
            <p style="font-size: 14px; color: #6B7280; line-height: 1.5;">Este código expirará en 10 minutos. Si tú no solicitaste este cambio, por favor ignora este mensaje y asegúrate de que tu cuenta esté segura.</p>
            <hr style="border: 0; border-top: 1px solid #E5E7EB; margin: 30px 0;" />
            <p style="font-size: 12px; color: #9CA3AF; text-align: center; margin: 0;">ReservaFit App &copy; 2026. Todos los derechos reservados.</p>
          </div>
        `,
